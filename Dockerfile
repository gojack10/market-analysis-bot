...
# Build & install TA-Lib from source with prefix=/usr/local
...
# Verify where TA-Lib is installed (should be under /usr/local)
RUN ls -la /usr/local/include/ta-lib || echo "TA-Lib headers not in /usr/local/include/ta-lib" \
...
# Install Python dependencies (ensuring TA_* vars are active during build)
